...
button1.position.set(80, 15, -55); // Position of the button in 3D space
scene.add(button1);
...
const text0Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("السبورة البيضاء ", "white", fontn),
    transparent: true,
});
...
const text1Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("قتح السبورة ", "white", fontn),
    transparent: true,
});
...
text1Plane.position.set(80, 17, -55); // Position above Button 1
scene.add(text1Plane);

// Add text to Button 2
const text2Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("أدوات السبورة ", "white", fontn),
    transparent: true,
});
...
text2Plane.position.set(90, 17, 6); // Position above Button 2
scene.add(text2Plane);

// Add text to Button 3
const text3Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("أمكانيات  ", "white", fontn),
    transparent: true,
});
...
text3Plane.position.set(65, 17, 50); // Position above Button 2
scene.add(text3Plane);

// Add text to Button 4
const text4Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("تطبيق السبورة  ", "white", fontn),
    transparent: true,
});
...
text4Plane.position.set(-65, 17,50); // Position above Button 2
scene.add(text4Plane);

// Add text to Button 5
const text5Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("العمل من سطح المكتب ", "white", fontn),
    transparent: true,
});
...
text5Plane.position.set(-90, 17, 6); // Position above Button 2
scene.add(text5Plane);

// Add text to Button 6
const text6Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("أدوات سطح المكتب ", "white", fontn),
    transparent: true,
});
...
text6Plane.position.set(-80, 17,-55); // Position above Button 2
scene.add(text6Plane);

// Add text to Button 7
const text7Material = new THREE.MeshBasicMaterial({
    map: createTextTexture("الصور والأضافات", "white", fontn),
    transparent: true,
});
...
text7Plane.position.set(22, 17, -75); // Position above Button 2
...
window.addEventListener("click", (event) => {
    ...
    const intersects = raycaster.intersectObjects([button0,button1, button2,button3,button4,button5,button6,button7]);
